...
unsigned int	secondCount[1000][5] = {0};
...
unsigned long * getSecondaryCount(char *lne, int count) {

    unsigned long *secondaryColors = malloc(sizeof(unsigned long) * 5);
	
    int record = 0, c = 0;

    for (int i = 0; i < strlen(lne); i++)
    {
		if (lne[i] >= '0' && lne[i] <= '9')
		{
			secondaryColors[c] = line[i] - '0';
			c++;
		}
	}
	secondaryColors[c] = 0;
...
void	getInside(int bagHash) {
    int c = 0;
    int i = searchHash(bagHash);
    
	if (i == -1)
		return;
	while (secondCount[i][c] != 0)
	{
		for (int v = 0; v < secondCount[i][c]; v++)
		{
			total++;
			getInside(secondList[i][c]);
		}
		c++;
	}
}
